...
def chip_clock_group(label, options, key, cols=4):
    ...
    for i, opt in enumerate(options):
        with col_list[i % cols]:
            ...
            if st.button(opt, key=f"{key}_{opt}", type="primary" if is_selected else "secondary", use_container_width=True):
...
get_ss("selected_call_types", set())
...
with st.sidebar:
    ...
    if st.button("Load/Create Game"):
        if selected_game:
            ...
            ss["game_meta"][selected_game] = {"opponent": "N/A", "type": "Game"}
    ...
    if ss["current_game"]:
        ...
        ss["game_meta"][ss["current_game"]]["quarter"] = st.selectbox("Quarter:", QUARTERS, index=QUARTERS.index(current_quarter))
        ...
        # Chip-style clock presets
        st.markdown("---")
        st.markdown("#### Clock Presets")
        chip_clock_group("Quarter Start", ["10:00", "08:00", "05:00", "02:00"], "clock_presets_Q", cols=4)
        chip_clock_group("Shot Clock", ["24", "14", "10", "05", "00"], "clock_presets_SC", cols=5)
        ...
        ss["credit_play"] = st.selectbox("Credit Play (for PPP):", options=[""] + ss["ms_plays"])
...
if not ss["current_game"]:
    ...
    with col2:
        ...
        ss["selected_call_types"] = chip_check_group(None, ["Half Court", "ATO", "BLOB", "SLOB", "Press Break"], "call_types", cols=3, small=True)
    with col3:
    ...
    def build_row_from_ui(outcome_text: str):
        ...
        call_types_str = join_pipe(ss["selected_call_types"] or ["Half Court"])
    ...
    if ss.get("pending_action"):
        with st.form("pending_form", border=True):
            st.write(
                ...
                f"| Call Type(s): **{ ' | '.join(ss['selected_call_types'] or ['Half Court']) }** | 2nd: **{second_chance}**"
    ...
    with DL:
        if df.empty:
            ...
            if not cred.empty:
                ...
                st.dataframe(grp_credit[["Credit Play","Attempts","Points","PPP","Success%"]], use_container_width=True, hide_index=True)
                ppp_chart = alt.Chart(grp_credit).mark_bar().encode(
                    x=alt.X("Credit Play", sort="-y", title=None), y=alt.Y("PPP", title="Points Per Possession"),
                    tooltip=["Credit Play", "PPP", "Attempts", "Success%"]
                ).properties(title="PPP by Play", height=300)
            ...
            if not vis_exploded.empty:
                grp_all = vis_exploded.groupby("Plays", dropna=False).agg(
                    Frequency=("Timestamp", "count"), Successes=("Success", lambda s: sum(_success_to_bool(x) for x in s))
                ).reset_index()
                ...
                st.dataframe(grp_all[["Plays","Frequency","Frequency%","Success%"]], use_container_width=True, hide_index=True)

    with DR:
        if not df.empty:
            freq_chart = alt.Chart(grp_all).mark_bar().encode(
                x=alt.X("Plays", sort="-y", title=None), y=alt.Y("Frequency", title="Frequency"),
                tooltip=["Plays", "Frequency", "Frequency%"]
            ).properties(title="Frequency of Plays", height=300)
            st.altair_chart(freq_chart, use_container_width=True)
            success_chart = alt.Chart(grp_all).mark_bar().encode(
                x=alt.X("Plays", sort="-y", title=None), y=alt.Y("Success%", title="Success Rate", axis=alt.Axis(format=".0%")),
                tooltip=["Plays", "Success%", "Frequency"]
            ).properties(title="Success Rate by Play", height=300)
    ...
    if df.empty:
        ...
        edited_df = st.data_editor(
            df_editor, column_order=GAME_HEADERS,
            column_config={
            ...
            use_container_width=True, hide_index=True, num_rows="dynamic", key="data_editor_table"
        )
